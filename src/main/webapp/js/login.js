document.addEventListener("DOMContentLoaded", () => {
    ...
    // 1. Các sự kiện đóng mở cơ bản
    if (btn) btn.onclick = () => modal.style.display = "block";
    if (closeBtn) closeBtn.onclick = () => modal.style.display = "none";
    window.onclick = (e) => { if (e.target === modal) modal.style.display = "none"; };

    // 2. Chuyển đổi giữa Đăng nhập và Đăng ký
    ...
    // 3. Xử lý logic Đăng ký (Local Storage)
    if (registerBox) {
    ...
    // 4. LOGIC TỰ ĐỘNG MỞ MODAL (Phải nằm ngoài các IF trên để luôn chạy)
    ...
    if (urlParams.get('requireLogin') === 'true') {
        if (modal) {
            ...
            const loginTitle = loginBox ? loginBox.querySelector("h2") : null;
            if (loginTitle) {
                loginTitle.textContent = "Vui lòng đăng nhập để đặt bàn";
